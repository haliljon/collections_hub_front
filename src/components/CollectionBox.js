                ...
                <div class="col-8 mt-4">
                    <div class={`card text-center shadow mb-5 bg-${isDarkMode ? 'dark' : 'white'} rounded`}>
                        ...
                        <div class="card-body">
                            ...
                            <Link to={`/collection/${collection.id}`} className={`btn btn-${isDarkMode ? '' : 'outline-'}success m-1`} state={{ collection }}>View</Link>
                            {(user.id === current_user || isAdmin()) && (<>
                                <button type="button" class={`btn btn-${isDarkMode ? '' : 'outline-'}danger m-1`} onClick={handleDelete}>Delete</button></>)}
                        </div>
