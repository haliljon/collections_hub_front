                ...
                <div class="col-sm-12 col-md-8 mt-4 mx-auto">
                    <div class={`card text-center shadow mb-5 bg-${isDarkMode ? 'dark' : 'white'} rounded`}>
                        <div class={`card-header fst-italic text-${isDarkMode ? 'white' : 'dark'}`}>
                            {t('by_someone')} {(user.id == current_user ? t("You") : user.username)}
                        </div>
                        <div class="card-body">
                            ...
                            <Link to={`/collection/${collection.id}`} className={`btn btn-${isDarkMode ? '' : 'outline-'}success m-1`} state={{ collection }}>{t('View')}</Link>
                            {(user.id === current_user || isAdmin()) && (<>
                                <button type="button" class={`btn btn-${isDarkMode ? '' : 'outline-'}danger m-1`} onClick={handleDelete}>{t('Delete')}</button></>)}
                        </div>
                        <div class={`card-footer text-${isDarkMode ? 'white' : 'muted'}`}>
                            {t('Created at')} {timeFormatter(collection.created_at)}, {t('edited at')} {timeFormatter(collection.updated_at)}
                        </div>
                    </div>
                </div>
            </div>
